...
def get_args_parser():
    ...
    parser.add_argument('--decay_step', type=int, default=2,
                        help='how many iter to decay lr')
    ...
    parser.add_argument('--log_dir', default='./output_test',
...
def main(args):
    ...
    transform_train = transforms.Compose([
        transforms.RandomResizedCrop(args.input_size, scale=(0.7, 1.0), interpolation=3),
        transforms.ToTensor(),
    ...
    transform_val = transforms.Compose([
        ...
        transforms.Resize((512, 512)),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
    dataset_train = NeRFDataset(os.path.join(args.data_path), transform=transform_train)
    ...
    head = ['head.weight', 'head.bias', 'norm.weight', 'norm.bias', 'norm.running_mean', 'norm.running_var', 'norm.num_batches_tracked']
    backbone_p = []
    head_p = []
    for name, param in model_without_ddp.named_parameters():
        if name not in head:
            ...
            head_p.append(param)
    params = [{'params': backbone_p, 'lr': 0.01 * args.lr},
